...
# Forțăm backend-ul TensorFlow în mod silențios
...
with st.sidebar:
    ...
    chart_data = {
        "Stare": [t.get("finished", "Gata"), t.get("remaining", "Rămas")],
        "Sesiuni": [done, 9 - done],
    }
    st.bar_chart(data=chart_data, x="Stare", y="Sesiuni")

st.header(
    t.get("details_header", "Sesiunea {}").format(current_id, COURSES[current_id][lang])
)

chk = st.checkbox(
    t.get("checkbox_label", "Completat"), value=progres.get(str(current_id), False)
)
...
if content:
    t1, t2, t3, t4 = st.tabs(
        [
            t.get("tab_glossary", "Glosar"),
            t.get("tab_code", "Cod"),
            t.get("tab_exercises", "Exerciții"),
            t.get("tab_sandbox", "Sandbox"),
        ]
    )
    ...
    with t4:
        st.markdown(t.get("sandbox_header", "### Keras 3 Async Sandbox"))
        code = st.text_area(
            "Editor:",
        ...
        if st.button(t.get("run_btn", "Execută")):
            # Soluție Keras Explorer: Rularea într-un Thread izolat cu Timeout de 10s pentru a preveni blocarea Streamlit
            output_holder = {"output": "", "error": None}
            ...
            def run_isolated_code():
                ...
                try:
                    # Injectăm implicit instanțele importante Keras pentru utilizator
                    exec_env = {
                        "keras": __import__("keras"),
                        "ops": __import__("keras.ops"),
                    }
                    exec(code, exec_env, exec_env)
                    output_holder["output"] = redirected_output.getvalue()
                except Exception as e:
                    output_holder["error"] = e
                finally:
            ...
            if worker_thread.is_alive():
                st.error(
                    "⚠️ Execuția a durat mai mult de 10 secunde. Thread-ul a fost izolat pentru a preveni blocarea interfeței."
                )
            elif output_holder["error"]:
                st.error(t.get("exec_err", "Eroare: {}").format(output_holder["error"]))
            else:
                st.code(
                    output_holder["output"]
                    if output_holder["output"].strip()
                    else "Codul a rulat cu succes, dar nu a printat nimic.",
                    language="text",
                )
else:
