...
## 🗺️ 1. Diagrama de Arhitectură și Circulație a Datelor

```text
       [ UTILIZATOR / INTERFAȚĂ GRAFICĂ (app.py) ]
          │                   │                 │
     (1) Căutare         (2) Execuție      (3) Bifare
         RAG                 Cod             Progres
          │                   │                 │
          ▼                   ▼                 ▼
   [ tools.py ]         [ exec() ]       [ progres.json ]
          │                   │                 │
          ▼                   ▼                 └─ State Hydration
   [ database.json ]    [ StringIO() ]             (Meniu Dropdown)
          │                   │
          ▼                   ▼
   [ schema.py ]        [ local_scope ]
   (Pydantic Validation)      │
          │                   └─ If "history" ──► [ st.line_chart ]
          ▼
   [ Răspuns JSON ] ──► (Randare pe ecran)
```

---

## 🔍 2. Fluxul Căutării Semantice (Simulator RAG)
...
3. **Extragere**: Instrumentul brutește direct din baza de date centrală locală `data/database.json`.
...
## 💻 3. Fluxul de Execuție în Sandbox (⚡ Python Sandbox)
...
## 💾 4. Managementul Persistenței Progresului

* **Input**: Utilizatorul bifează starea unei sesiuni în zona centrală a ecranului.
...
## 🌐 5. Managementul Internaționalizării (i18n System)
...
* **State Hydration**: La fiecare refresh (`st.rerun()`), toate tab-urile, titlurile, butoanele, mesajele de succes și indicatorii grafici sunt traduse instantaneu în memorie înainte de randarea pe ecran, garantând performanță maximă (latență structurală zero).
